...
std::unique_ptr<PlayerModel> ConcreteFactory::createPlayer(float x, float y)
{
        ...
        std::unique_ptr<ScoreView> scoreView =
            std::make_unique<ScoreView>(std::pair<int, int>(gameWindow->getSize().x / 2, 30), gameWindow, scoreFont);
        std::unique_ptr<HPView> HP_view = std::make_unique<HPView>(
            std::pair<int, int>(playerModel->getPosition().first,
                                playerModel->getPosition().second + playerModel->getHeight() * 2),
            gameWindow, scoreFont, playerModel->getMaxHp());
...
std::unique_ptr<PlatformModel> ConcreteFactory::createPlatform(float x, float y, PlatformType::Type ptype)
{
        ...
        std::unique_ptr<EntityView> platformView =
            std::make_unique<PlatformView>(ptype, platformModel->getWidth(), platformModel->getHeight(), gameWindow);
                                                         ...
                                                         BonusType::Type btype)
{
        ...
        if (btype == BonusType::spring) {
                bonusView = std::make_unique<BonusView>(gameTextures["springTexture"], bonusModel->getWidth(),
                                                        bonusModel->getHeight(), gameWindow);
        } else if (btype == BonusType::spike) {
                bonusView = std::make_unique<BonusView>(gameTextures["spikeTexture"], bonusModel->getWidth(),
                                                        bonusModel->getHeight(), gameWindow);
        } else if (btype == BonusType::heart) {
                bonusView = std::make_unique<BonusView>(gameTextures["heartTexture"], bonusModel->getWidth(),
                                                        bonusModel->getHeight(), gameWindow);
        } else if (btype == BonusType::jetpack) {
                bonusView = std::make_unique<BonusView>(gameTextures["jetpackTexture"], bonusModel->getWidth(),
                                                        bonusModel->getHeight(), gameWindow);
        }
...
std::unique_ptr<BulletModel> ConcreteFactory::createBullet(float x, float y, BulletType::Type bulletType)
{
        ...
        std::unique_ptr<BulletView> bulletView =
            std::make_unique<BulletView>(bulletModel->getWidth(), bulletModel->getHeight(), gameWindow, bulletType);
                                                         ...
                                                         EnemyType::Type etype)
{
        ...
        if (etype == EnemyType::weak) {
                enemyView = std::make_unique<EnemyView>(gameTextures["monster2Texture"], enemyModel->getWidth(),
                                                        enemyModel->getHeight(), etype, gameWindow);
        } else if (etype == EnemyType::strong) {
                enemyView = std::make_unique<EnemyView>(gameTextures["monster1Texture"], enemyModel->getWidth(),
                                                        enemyModel->getHeight(), etype, gameWindow);
        }
        std::unique_ptr<HPView> HP_view = std::make_unique<HPView>(
            std::pair<int, int>(enemyModel->getPosition().first,
                                enemyModel->getPosition().second + enemyModel->getHeight() * 2),
            gameWindow, scoreFont, enemyModel->getMaxHp());
