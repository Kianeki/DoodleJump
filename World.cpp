...
void World::generateRandomEntities() {
    ...
    while (!camera.screenFilled(lastPlatformPosition)) {
        ...
        std::unique_ptr<PlatformModel> newPlatform = factory->createPlatform(newPlatformX, newPlatformY,
                                                                             random->randomPlatformType());
        ...
        if ((*it).second != nullptr) {
            ...
            if ((*it).second->getType() == BonusType::spring) {
                ...
                if (checkPlatformCollision(bonusPos, bonusWidth, bonusHeight)) {
                    ...
                    //this will give player the normal jump speed and notify the player which platform the bonus is on
                ...
                if (checkBonusCollision((*it).second) && activeBonus == nullptr) {
                    ...
                    player->platformCollide((*it).first);
                    //this will give player the normal jump speed and notfy the player which platform the bonus is on
                    (*it).second->applyToPlayer(player);
