...
namespace PlatformType {
    enum Type {
        staticP = 200, temporaryP = 0, horizontalP = 100, verticalP = 75
    }; //these int values are for score calc
}
namespace BonusType {
    enum Type {
        none = 0, spring = 500, jetpack = 2000
    };  //these int values are for score calc
}
class Random {
public:
    ...
    float randomPlatformY(float platformHeight,
                          float minPlatformDistance) { // platformHeight is the thickness of the platform
